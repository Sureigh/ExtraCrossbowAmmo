...
self.weapon id here.AMMO_PICKUP = {0.5, 0.85} --ammo pickup is randomized between the two values.
self.weapon id here.can_shoot_through_enemy = false --true or false.
self.weapon id here.can_shoot_through_wall = false --true or false.
self.weapon id here.can_shoot_through_shield = true --true or false.
self.weapon id here.armor_piercing_chance = 1 --1 for always piercing, 0 for no piercing. can be any value inbetween for a chance of piercing.
self.weapon id here.stats.alert_size = 20 --the distance at which your gun shots are audible, matters only really in stealth. similarly to suppression, 1 is the maximum value, 20 is the minimum value (the gun is quiet as far as the game is concerned). distance table in READ ME FIRST.txt.
